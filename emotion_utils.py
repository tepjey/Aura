...
def provide_feedback(detected_emotion):
    feedback = f"Your current emotion is {detected_emotion}. {get_recommendation(detected_emotion)}"
    engine = pyttsx3.init()
    engine.say(feedback)  # Verbal feedback
    engine.runAndWait()
